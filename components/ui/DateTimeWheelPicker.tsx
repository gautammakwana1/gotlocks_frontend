...
const partsToDate = (p: Parts): Date => {
    const h24 = (p.h12 % 12) + (p.isPM ? 12 : 0);
    const maxDay = daysInMonth(p.year, p.monthIdx);
    return new Date(p.year, p.monthIdx, Math.min(p.day, maxDay), h24, p.minute, 0, 0);
};
...
export default function DateTimeWheelPicker({
    ...
    const years = useMemo(() => {
        ...
        const low = Math.min(minYear ?? now - 1, parts.year);
        const high = Math.max(maxYear ?? now + 10, parts.year);
    ...
    const commit = () => {
        onChange(partsToDate(parts).toISOString());
        requestClose();
    };
    ...
    return (
        <div className={`block ${className ?? ""}`}>
            ...
            >
                <span className={value ? "text-white" : "text-gray-500"}>{formatLabel(value)}</span>
            ...
            {mounted && (
                ...
                >
                    ...
                    />
                    <div
                        className={`mx-3 relative z-10 w-full overflow-hidden rounded-t-3xl border border-sky-400/30 bg-gradient-to-br from-white/10 via-white/5 to-white/[0.03] shadow-[0_-12px_40px_rgba(0,0,0,0.6),0_0_22px_-6px_rgba(59,130,246,0.45)] backdrop-blur transition-all duration-300 ease-out will-change-transform motion-reduce:transition-none sm:max-w-md sm:rounded-3xl ${visible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-6"
                            }`}
                    >
                        <div className="flex items-center justify-between gap-3 border-b border-white/10 px-4 py-3">
                            ...
                            <span className="truncate text-sm font-medium text-sky-200">
                                {formatLabel(partsToDate(parts).toISOString())}
                            </span>
                        </div>
                        ...
                        <div className="relative px-3 py-3">
                            ...
                            <div className="relative flex gap-1 text-sm">
                                <WheelColumn
                                    ...
                                    onIndexChange={(i) => setParts((p) => clampDay({ ...p, monthIdx: i }))}
                                    ariaLabel="Month"
                                />
                                <WheelColumn
                                    items={days.map(String)}
                                    index={parts.day - 1}
                                    onIndexChange={(i) => setParts((p) => ({ ...p, day: i + 1 }))}
                                    ariaLabel="Day"
                                />
                                <WheelColumn
                                    items={years.map(String)}
                                    index={Math.max(0, years.indexOf(parts.year))}
                                    onIndexChange={(i) => setParts((p) => clampDay({ ...p, year: years[i] }))}
                                    ariaLabel="Year"
                                />
                                <WheelColumn
                                    items={range(1, 12).map(String)}
                                    index={parts.h12 - 1}
                                    onIndexChange={(i) => setParts((p) => ({ ...p, h12: i + 1 }))}
                                    ariaLabel="Hour"
                                />
                                <WheelColumn
                                    items={range(0, 59).map((m) => String(m).padStart(2, "0"))}
                                    index={parts.minute}
                                    onIndexChange={(i) => setParts((p) => ({ ...p, minute: i }))}
                                    ariaLabel="Minute"
                                />
                                <WheelColumn
                                    items={["AM", "PM"]}
                                    index={parts.isPM ? 1 : 0}
                                    onIndexChange={(i) => setParts((p) => ({ ...p, isPM: i === 1 }))}
                                    ariaLabel="AM or PM"
                                />
                            </div>
                        </div>
